...
def create_supervisor_system():
    ...
    def supervisor(state: SupervisorState) -> dict:
        ...
        # FIX: Đảm bảo tin nhắn cuối cùng là HumanMessage
        # Chuyển đổi tin nhắn cuối cùng thành HumanMessage nếu cần
        messages = [SystemMessage(content=system_prompt)]
        
        # Thêm tất cả tin nhắn từ state, nhưng đảm bảo tin nhắn cuối là HumanMessage
        for msg in state["messages"]:
            if isinstance(msg, AIMessage) and "[Supervisor]" in msg.content:
                # Bỏ qua tin nhắn của supervisor để tránh loop
                continue
            messages.append(msg)
        
        # Nếu tin nhắn cuối không phải HumanMessage, thêm một tin nhắn chuyển tiếp
        if messages and not isinstance(messages[-1], HumanMessage):
            messages.append(HumanMessage(
                content="Continue with the task. What should be done next?"
            ))
