...
llm = init_chat_model("claude-sonnet-4-6", temperature=0.0)
...
def demo_basic_routing():
    def classify_query(state: RouterState) -> dict:
        ...
        return {"query_type": response.content.lower().strip()} # pyright: ignore[reportAttributeAccessIssue]
    ...
    for query in queries:
        result = app.invoke({"query": query}) # pyright: ignore[reportArgumentType]
        print(f"Query: {query}")
...
def demo_conditional_loop():
    ...
    def evaluate_quality(state: QualityState) -> dict:
        ...
        try:
            score = int(response.content.strip()) # pyright: ignore[reportAttributeAccessIssue]
        except:
            score = 2
...
def demo_multi_path_routing():
    ...
    def analyze_task(state: TaskState) -> dict:
        ...
        return {
            "urgency": urgency_response.content.lower().strip(), # pyright: ignore[reportAttributeAccessIssue]
            "complexity": complexity_response.content.lower().strip(), # pyright: ignore[reportAttributeAccessIssue]
        }
    ...
    for task in tasks:
        result = app.invoke({"task": task}) # pyright: ignore[reportArgumentType]
        print(f"Task: {task}")
