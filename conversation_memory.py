...
def count_tokens_with_tiktoken(messages):
    """Đếm token cho list messages bằng tiktoken."""
    if not messages:
        return 0
    
    # Dùng cl100k_base cho Anthropic models
...
def demo_message_trimming():
    ...
    trimmed = trim_messages(
        ...
        max_tokens=60,
        strategy="last",
        token_counter=count_tokens_with_tiktoken,
    ...
    print("\nTrimmed messages:")
    for msg in trimmed:
        ro = type(msg).__name__
        print(f"Role original: {ro}")
...
def demo_windowed_memory():
    ...
    for i, msg in enumerate(exchanges, 1):
        ...
        print(f"History: {store['windowed_test']}")
...
def demo_summary_memory():
    ...
    # --- Setup ---
    summary_llm = ChatAnthropic(model_name="claude-sonnet-4-6", temperature=0)
    chat_llm = ChatAnthropic(model_name="claude-sonnet-4-6", temperature=0.7)
