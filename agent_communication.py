...
def create_message_passing_pipeline():
    ...
    def researcher(state: MessagePassingState) -> dict:
        ...
        return {
            "messages": [
                HumanMessage(  # Store as HumanMessage
    ...
    def fact_checker(state: MessagePassingState) -> dict:
        ...
        # Convert all messages to HumanMessage except SystemMessage
        messages_for_llm = []
        for msg in state["messages"]:
            if isinstance(msg, SystemMessage):
                messages_for_llm.append(msg)
            else:
                # Convert any non-SystemMessage to HumanMessage
                content = msg.content if hasattr(msg, 'content') else str(msg)
                name = getattr(msg, 'name', None)
                if name:
                    messages_for_llm.append(HumanMessage(content=content, name=name))
                else:
...
def demo_message_passing():
    ...
    for msg in result["messages"]:
        if isinstance(msg, HumanMessage) and hasattr(msg, 'name') and msg.name:
            print(f"{msg.content}\n")
        elif isinstance(msg, HumanMessage) and not hasattr(msg, 'name'):
            print(f"{msg.content}\n")
