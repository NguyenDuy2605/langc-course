...
import tempfile
import shutil #copy fiel/foleder

load_dotenv()

#embeddings_model = VoyageAIEmbeddings(model="voyage-large-2")
embeddings_model = HuggingFaceEmbeddings(
        model_name="all-MiniLM-L6-v2",  # Model nhẹ, tốc độ nhanh
        model_kwargs={'device': 'cpu'},  # Dùng CPU, nếu có GPU thì 'cuda'
...
def pinecone_vector_store():
    ...
    # Cách 2: Tạo từ documents (giống với Chroma)
    # vector_store = PineconeVectorStore.from_documents(
    #     documents=SAMPLE_DOCS,
    #     embedding=embeddings_model,
    #     index_name=os.getenv("PINECONE_INDEX_NAME")
    # )
    # Thực hiện similarity search - giống hệt Chroma

    print(
            f"Vector store created {vector_store.index.describe_index_stats()} documents and persisted."
        )
    query = "What is LangChain?"
    ...
    for i, doc in enumerate(results):
        print(
            f"Result {i+1}: {doc.page_content} (Source: {doc.metadata.get('source', 'Unknown')})"
        )
