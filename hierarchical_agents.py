...
def build_research_team() -> StateGraph:
    ...
    def research_lead(state: TeamState) -> dict:
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are the research lead..."),
            MessagesPlaceholder(variable_name="research_findings"),
            ("human", "Please synthesize the above findings into a brief.")  # ← Human cuối
        ])
        
        # Format messages
        formatted_messages = prompt.format_messages(
            research_findings=state["messages"]  # Chỉ lấy findings, không lấy cả conversation
        )
