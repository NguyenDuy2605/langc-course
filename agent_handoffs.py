...
llm = ChatAnthropic(model_name="claude-sonnet-4-6", temperature=0) # type: ignore
...
def create_customer_service_system():
    ...
    def sales_agent(state: HandoffState) -> dict:
        ...
        # Chỉ lấy HumanMessage, bỏ AIMessage của triage
        human_messages = [m for m in state["messages"] if isinstance(m, HumanMessage)]
        response = llm.invoke([SystemMessage(content=system), *human_messages])
